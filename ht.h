...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if (double(NoOfInserts)/table_.size() >= alphaTreshold) {
        resize();
        insert(p);
    }
    else{

        int location = this->probe(p.first);
        if(location == npos){
            throw std::logic_error("can not insert");
        }
        
        else if (location != npos) {
            if (table_[location] != nullptr && !table_[location]->deleted){
                table_[location]->item = p;
            }
            else if (table_[location] == nullptr){
...
{
        int location = this->probe(key);
        
        if (location != npos && table_[location] != nullptr) {
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    for (int i = 0; i < newTable.size(); i++) {
        table_[probe(newTable[i]->item.first)] = newTable[i];
        NoOfInserts++;
     }
